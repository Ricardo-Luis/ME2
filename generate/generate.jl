...
function sidebar_code(book_model)
    ...
    $(map(enumerate(book_model)) do (chapter_number, chap)
		...
		$(map(enumerate(chap.contents)) do (section_number, section)
		    ...
		    @htl("""
		    <a class="sidebar-nav-item {{ispage /$notebook_name/}}active{{end}}" href="$(SLASH_PREPATH)/$notebook_id/"><b>$(chapter_number).$(section_number)</b> - <strong>$(section.name)</strong></a>
		    """)
		end)
		""")
	end)
